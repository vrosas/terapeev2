...
import { AlertCircle, ArrowRight, ArrowUpRight, Ban, Calendar, Check, CheckCircle2, Clock, CreditCard, DollarSign, MessageSquare, MoreHorizontal, RefreshCw, Timer, TrendingDown, TrendingUp, Users, Video, XCircle } from 'lucide-react'
import {
...
const STATUS_MAP = {
  ...
  confirmed:    { label: 'Confirmado',   color: T.success,  bg: 'rgba(22,163,74,0.10)',  icon: CheckCircle2 },
  cancelled:    { label: 'Cancelado',    color: T.error,    bg: 'rgba(220,38,38,0.10)',  icon: XCircle },
  no_show:      { label: 'Faltou',       color: T.error,    bg: 'rgba(220,38,38,0.10)',  icon: Ban },
}
    ...
    <div style={{
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, color: T.n700, marginTop: 2 }}>
          ...
          {p.name === 'receita' ? 'Receita' : p.name === 'meta' ? 'Meta' : p.name === 'confirmados' ? 'Confirmados' : 'Total'}:
...
function AppointmentRow({ apt, idx }) {
  const time = apt.start_time
    ...
    : apt.time || '--:--'
  const duration = apt.duration_minutes || apt.professional?.session_duration || 50
  const patientName = apt.patient?.full_name || apt.patient_name || 'Paciente'
  const profName = apt.professional?.full_name || ''
  const service = apt.service_type || apt.type || 'Consulta'
  const status = apt.status || 'pendente'
  const isOnline = apt.modality === 'online' || apt.is_online

  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px',
      borderBottom: `1px solid ${T.n100}`, transition: 'background 150ms',
      opacity: status === 'cancelado' || status === 'cancelled' ? 0.5 : 1,
      ...
      <div style={{
        ...
        background: STATUS_MAP[status]?.color || T.n400,
        boxShadow: `0 0 0 3px ${STATUS_MAP[status]?.bg || T.n200}`,
      }} />
...
export default function Dashboard() {
  ...
  const overdueCharges = useMemo(() =>
    allCharges.filter((c) => c.status === 'overdue' || c.status === 'vencido' ||
      (c.due_date && new Date(c.due_date) < new Date() && c.status !== 'paid' && c.status !== 'pago')),
    [allCharges]
  )

  const confirmed = appointments.filter((a) => ['confirmado', 'confirmed', 'scheduled'].includes(a.status)).length
  const pending = appointments.filter((a) => ['pendente', 'sem_resposta'].includes(a.status)).length
  const totalApts = appointments.length
  const userName = profile?.full_name?.split(' ').slice(0, 2).join(' ') || 'Doutor(a)'
  ...
  const revenueChart = [
    ...
    { month: 'Jan', receita: stats?.month_revenue || 26200, meta: 26000 },
  ]
  ...
  const overdueTotal = overdueCharges.reduce((s, c) => s + (c.amount || 0), 0)

  if (statsLoading && aptsLoading) return <LoadingSpinner />
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1280 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        <StatCard icon={Calendar} label="Consultas hoje" value={totalApts}
          subtext={`${confirmed} confirmadas · ${pending} pendentes`}
          color={T.primary500} delay={0.05} />
        <StatCard icon={CreditCard} label="Receita do mês" value={`R$ ${(stats?.month_revenue || 0).toLocaleString('pt-BR')}`}
          subtext={`${stats?.month_completed || 0} sessões realizadas`} trend="+8%" trendUp
          color={T.warning} delay={0.1} />
        <StatCard icon={AlertCircle} label="Cobranças atrasadas" value={`R$ ${overdueTotal.toLocaleString('pt-BR')}`}
          subtext={`${overdueCharges.length} faturas vencidas`}
          color={T.error} delay={0.15} />
        <StatCard icon={CheckCircle2} label="Pacientes ativos" value={stats?.active_patients || 0}
          subtext="Cadastrados na clínica" trend="+5%" trendUp
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: T.n900, letterSpacing: '-0.01em' }}>
                  R$ {(stats?.month_revenue || 0).toLocaleString('pt-BR')}
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={weeklyChart} barGap={4} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
          ...
          <div style={{
            ...
            ) : overdueCharges.slice(0, 5).map((inv, i) => (
              <div key={inv.id || i} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: T.n900 }}>
                    {inv.patient?.full_name || 'Paciente'}
                  </div>
                  <div style={{ fontSize: 12, color: T.n400, marginTop: 1 }}>
                    {inv.description || 'Sessão'} · Venc. {inv.due_date ? new Date(inv.due_date).toLocaleDateString('pt-BR') : '--'}
                  </div>
                </div>
          ...
          <div style={{
            background: T.n0, borderRadius: T.radiusLg, boxShadow: T.shadowSoft,
            border: `1px solid ${T.n200}`, padding: '20px',
            animation: 'fadeSlideUp 0.4s ease 0.35s both',
          }}>
            <h2 style={{ fontSize: 16, fontWeight: 600, marginBottom: 14 }}>Ações rápidas</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { icon: Calendar, label: 'Novo agendamento', color: T.primary500, target: 'agenda' },
                { icon: Users, label: 'Cadastrar paciente', color: T.success, target: 'pacientes' },
                { icon: DollarSign, label: 'Gerar fatura', color: T.warning, target: 'financeiro' },
                { icon: MessageSquare, label: 'Enviar mensagem', color: T.info, target: 'mensagens' },
              ].map((action, i) => (
                <button key={i} onClick={() => navigate(action.target)} style={{
