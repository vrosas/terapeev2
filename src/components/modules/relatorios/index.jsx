...
import {
  Activity, AlertCircle, AlertTriangle, ArrowDownRight, ArrowUpRight, Award,
  Bell, Briefcase, Calendar, Check, CheckCircle2, ChevronDown, ChevronLeft,
  ChevronRight, Clock, CreditCard, DollarSign, Download, Edit3, Eye, FileText,
  Filter, Hash, Heart, Home, LayoutDashboard, Loader2, Mail, MoreHorizontal,
  Phone, Plus, Printer, RefreshCw, Search, Settings, Shield, Star,
  TrendingDown, TrendingUp, User, Users, X, Zap
} from 'lucide-react'
import {
  Area,
  AreaChart,
...
import { T } from '@/utils/theme'
  ...
  Check, ChevronDown, TrendingUp, TrendingDown,
  Mail, Phone, Award, Briefcase, Activity,
  Shield, Heart, User, Zap, Home,
  Eye, Filter, Printer, RefreshCw, ArrowUpRight, ArrowDownRight,
  CreditCard, MoreHorizontal, Star, AlertTriangle, Hash
...
const BY_PROFESSIONAL = [
  {name:"Dra. Ana Costa",atendimentos:68,receita:12400,pacientes:34,ocupacao:85,color:T.primary500},
  {name:"Dr. Carlos Lima",atendimentos:92,receita:14800,pacientes:28,ocupacao:92,color:T.success},
  {name:"Dra. Beatriz Rocha",atendimentos:55,receita:8200,pacientes:22,ocupacao:72,color:T.warning},
  {name:"Dr. Ricardo Alves",atendimentos:42,receita:6800,pacientes:18,ocupacao:65,color:T.purple},
  {name:"Dra. Marina Souza",atendimentos:30,receita:7500,pacientes:15,ocupacao:58,color:T.teal},
];

const BY_SERVICE = [
  {name:"Psicoterapia Individual",qtd:98,receita:17640,color:T.primary500},
  {name:"Fisioterapia Geral",qtd:72,receita:10800,color:T.success},
  {name:"Fonoaudiologia Infantil",qtd:38,receita:4940,color:T.warning},
  {name:"Terapia Ocupacional",qtd:28,receita:4480,color:T.purple},
  {name:"Avaliação Neuropsicológica",qtd:12,receita:5400,color:T.teal},
  {name:"Fisioterapia Esportiva",qtd:22,receita:3960,color:T.orange},
  {name:"Terapia de Casal",qtd:15,receita:4200,color:T.pink},
];

const BY_PAYMENT = [
  {name:"Pix",value:35,color:T.teal},
  ...
  {name:"Transferência",value:8,color:T.purple},
];
...
const EXPENSE_CATS = [
  ...
  {name:"Software",value:446,color:T.purple},
...
function KPI({label,value,sub,trend,trendUp,icon:Icon,color,delay=0}){
  return(
    <div style={{background:T.n0,borderRadius:T.radiusLg,padding:"18px 20px",boxShadow:T.shadowSoft,border:`1px solid ${T.n200}`,display:"flex",alignItems:"center",gap:14,animation:`fadeSlideUp 0.4s ease ${delay}s both`,transition:"box-shadow 200ms,transform 200ms",cursor:"default"}}
      ...
      <div style={{width:44,height:44,borderRadius:12,background:`${color}14`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={20} color={color}/></div>
      <div style={{flex:1}}>
      ...
      {trend&&<div style={{display:"flex",alignItems:"center",gap:3,fontSize:12,fontWeight:600,color:trendUp?T.success:T.error,padding:"3px 8px",borderRadius:6,background:trendUp?"rgba(22,163,74,0.08)":"rgba(220,38,38,0.08)"}}>{trendUp?<TrendingUp size={12}/>:<TrendingDown size={12}/>}{trend}</div>}
    </div>
  );
}
  ...
  return(
    <div style={{background:T.n0,borderRadius:T.radiusLg,border:`1px solid ${T.n200}`,boxShadow:T.shadowSoft,overflow:"hidden",animation:`fadeSlideUp 0.35s ease ${delay}s both`}}>
      <div style={{padding:"18px 22px 0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div><div style={{fontSize:15,fontWeight:600}}>{title}</div>{subtitle&&<div style={{fontSize:12,color:T.n400,marginTop:2}}>{subtitle}</div>}</div>
        {action}
      </div>
  ...
  return(
    <div style={{padding:"24px 28px",maxWidth:1400}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,animation:"fadeSlideUp 0.3s ease both"}}>
        <div><h1 style={{fontSize:22,fontWeight:700}}>Relatórios</h1><p style={{fontSize:13,color:T.n400,marginTop:2}}>Análises de desempenho, faturamento e operação da clínica</p></div>
        <div style={{display:"flex",gap:8}}>
          <div style={{display:"flex",border:`1.5px solid ${T.n300}`,borderRadius:T.radiusMd,overflow:"hidden"}}>
            {[{id:"1m",label:"1M"},{id:"3m",label:"3M"},{id:"6m",label:"6M"},{id:"12m",label:"12M"}].map(p=>(
              <button key={p.id} onClick={()=>setPeriod(p.id)} style={{padding:"8px 14px",border:"none",cursor:"pointer",fontFamily:T.font,fontSize:12,fontWeight:period===p.id?600:400,color:period===p.id?T.primary500:T.n400,background:period===p.id?T.primary50:T.n0,transition:"all 150ms"}}>{p.label}</button>
            ))}
          </div>
          <button style={{padding:"9px 16px",borderRadius:T.radiusMd,border:`1.5px solid ${T.n300}`,background:T.n0,color:T.n700,fontFamily:T.font,fontSize:13,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><Download size={14}/> Exportar PDF</button>
          <button style={{padding:"9px 16px",borderRadius:T.radiusMd,border:`1.5px solid ${T.n300}`,background:T.n0,color:T.n700,fontFamily:T.font,fontSize:13,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><Printer size={14}/> Imprimir</button>
        </div>
      </div>
      ...
      <div style={{display:"flex",borderBottom:`2px solid ${T.n200}`,marginBottom:22}}>
        {tabs.map(t=>{const active=tab===t.id;return(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"12px 20px",border:"none",cursor:"pointer",fontFamily:T.font,fontSize:14,fontWeight:active?600:400,color:active?T.primary500:T.n400,background:"transparent",borderBottom:`2px solid ${active?T.primary500:"transparent"}`,marginBottom:-2,transition:"all 150ms"}}>{t.label}</button>
        )})}
      </div>
      ...
      {tab==="overview"&&(
        <div style={{animation:"fadeSlideUp 0.3s ease both"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:22}}>
            ...
            <KPI icon={Clock} label="Ocupação média" value={`${totals.ocupacaoMedia}%`} color={T.purple} delay={0.25}/>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16,marginBottom:16}}>
            ...
            <ChartCard title="Status dos atendimentos" subtitle="Janeiro 2025" delay={0.15}>
              <ResponsiveContainer width="100%" height={200}>
                <RPie>
                  <Pie data={BY_STATUS} cx="50%" cy="50%" innerRadius={50} outerRadius={78} paddingAngle={3} dataKey="value">
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <ChartCard title="Ocupação por profissional" subtitle="Janeiro 2025" delay={0.2}>
      ...
      {tab==="financial"&&(
        <div style={{animation:"fadeSlideUp 0.3s ease both"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16,marginBottom:16}}>
            ...
            <ChartCard title="Despesas por categoria" subtitle="Janeiro 2025" delay={0.15}>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <ChartCard title="Receita por forma de pagamento" subtitle="Janeiro 2025" delay={0.2}>
            <div style={{display:"flex",alignItems:"center",gap:32}}>
              <ResponsiveContainer width="40%" height={200}>
                <RPie>
                  <Pie data={BY_PAYMENT} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="value">
      ...
      {tab==="attendance"&&(
        <div style={{animation:"fadeSlideUp 0.3s ease both"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
            ...
            <ChartCard title="Receita por serviço" subtitle="Janeiro 2025" delay={0.15}>
              <ResponsiveContainer width="100%" height={240}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <ChartCard title="Ranking de profissionais" subtitle="Por volume de atendimentos" delay={0.2}>
              <div style={{background:T.n0,borderRadius:T.radiusMd,overflow:"hidden"}}>
        ...
        <div style={{animation:"fadeSlideUp 0.3s ease both"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16,marginBottom:16}}>
            <ChartCard title="Evolução da base de pacientes" subtitle="Últimos 6 meses" delay={0.1}>
              ...
              <div style={{display:"flex",gap:20,marginTop:4}}>
                <div style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:T.n400}}><div style={{width:12,height:3,borderRadius:2,background:T.primary500}}/> Ativos</div>
                <div style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:T.n400}}><div style={{width:12,height:3,borderRadius:2,background:T.success}}/> Novos</div>
              </div>
            </ChartCard>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            ...
            <ChartCard title="Indicadores de retenção" subtitle="Últimos 6 meses" delay={0.25}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {[
                  {label:"Retorno em 30 dias",value:"78%",desc:"Pacientes que retornam dentro de 1 mês",color:T.success,pct:78},
                  {label:"Retorno em 60 dias",value:"89%",desc:"Pacientes que retornam dentro de 2 meses",color:T.primary500,pct:89},
                  {label:"Abandono",value:"8%",desc:"Pacientes que não retornam há 90+ dias",color:T.error,pct:8},
                  {label:"Frequência média",value:"3.2x",desc:"Sessões por paciente por mês",color:T.warning,pct:80},
                ].map((m,i)=>(
